-- HLWAIT demo seed — full sample data for hlwait_demo schema only.
-- Safe to re-run: uses deterministic UUIDs + ON CONFLICT DO NOTHING / upsert where applicable.

-- Ensure demo schema is fully bootstrapped (no-op if already exists)
select hlwait.bootstrap_schema('hlwait_demo');

-- ---------------------------------------------------------------------------
-- Fixed IDs for reproducible references
-- ---------------------------------------------------------------------------
-- Roles (seeded by bootstrap; fetch ids)
-- Users
-- Categories, suppliers, customers, products, orders, etc.
...
begin
  select id into v_role_admin from hlwait_demo.roles where name = 'admin';
  select id into v_role_manager from hlwait_demo.roles where name = 'manager';
  select id into v_role_employee from hlwait_demo.roles where name = 'employee';

  -- Users
  insert into hlwait_demo.users (id, role_id, name, email, phone, is_active) values
    (v_user_admin,    v_role_admin,    'מנהל מערכת',   'admin@hlwait.demo',    '050-1000001', true),
    (v_user_manager,  v_role_manager,  'רונית כהן',    'ronit@hlwait.demo',    '050-1000002', true),
    (v_user_employee, v_role_employee, 'יוסי לוי',    'yossi@hlwait.demo',    '050-1000003', true)
  on conflict (id) do nothing;

  -- Categories
  insert into hlwait_demo.categories (id, name, description, sort_order) values
  ...
  -- Suppliers
  insert into hlwait_demo.suppliers (
  ...
  -- Supplier ledger
  insert into hlwait_demo.supplier_ledger (supplier_id, entry_date, doc_type, description, debit, credit) values
  ...
  -- Customers
  insert into hlwait_demo.customers (id, customer_code, name, phone, email, address, balance) values
  ...
  -- Products
  insert into hlwait_demo.products (
  ...
  insert into hlwait_demo.inventory (product_id, location, quantity) values
  ...
  -- Inventory movements
  insert into hlwait_demo.inventory_movements (
  ...
  -- Orders
  insert into hlwait_demo.orders (
  ...
  -- Order items
  insert into hlwait_demo.order_items (order_id, product_id, quantity, unit_price, line_total) values
  ...
  -- Payments
  insert into hlwait_demo.payments (customer_id, supplier_id, amount, payment_method, reference_type, reference_id, paid_at, notes) values
  ...
  -- Income
  insert into hlwait_demo.income (income_type, customer_id, order_id, amount, description, income_date) values
  ...
  -- Expenses
  insert into hlwait_demo.expenses (expense_type, supplier_id, employee_id, amount, description, expense_date, payment_status) values
  ...
  -- Tasks
  insert into hlwait_demo.tasks (title, description, assigned_user_id, due_date, status, priority) values
  ...
  -- Settings
  insert into hlwait_demo.settings (key, value) values
    ('business_name', '{"he": "HLWAIT Demo — קונדיטוריה", "en": "HLWAIT Demo Bakery"}'::jsonb),
